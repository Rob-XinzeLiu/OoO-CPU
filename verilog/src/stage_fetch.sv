...
module stage_if (
    ...
    // ---------------------------------------------------------------
    //  Direct-fetch inputs: testbench drives PC and memory data
    // ---------------------------------------------------------------
    input  ADDR         tb_PC,        // current fetch PC (from testbench)
    input  MEM_BLOCK    Imem_data,    // 64-bit memory line for that PC
 
    // ---------------------------------------------------------------
    //  Other control
    // ---------------------------------------------------------------
    input  MISPREDICT_PACKET mispredict_pack, // from execute stage
    input  logic [1:0]  fetch_req,    // from fetch buffer: slots available
    input  logic        stop_fetch,   // from retire stage
 
    // ---------------------------------------------------------------
    //  Outputs
    // ---------------------------------------------------------------
    output F_D_PACKET   if_packet [`N-1:0]
    // Note: Imem_addr output is gone — testbench owns the PC
);
 
    // ------------------------------------------------------------------
    //  How many valid instructions can we produce this cycle?
    //
    //  We can fetch two if:
    //   - the fetch address is 8-byte aligned  (tb_PC[2] == 0)
    //   - the fetch buffer asked for 2         (fetch_req == 2)
    //   - we are not stalled
    // ------------------------------------------------------------------
    logic two_valid_insts;
    assign two_valid_insts = (tb_PC[2] == 1'b0) && (fetch_req == 2'b10);
 
    // ------------------------------------------------------------------
    //  Instruction slot 0
    // ------------------------------------------------------------------
    assign if_packet[0].inst         = if_valid ? Imem_data.word_level[tb_PC[2]] : `NOP;
    assign if_packet[0].PC           = tb_PC;
    assign if_packet[0].NPC          = tb_PC + 4;
    assign if_packet[0].valid        = if_valid && !stop_fetch && (fetch_req >= 2'b01);
    assign if_packet[0].predict_taken = 1'b0;
    assign if_packet[0].predict_addr  = tb_PC + 4;
 
    // ------------------------------------------------------------------
    //  Instruction slot 1 (only valid on aligned two-wide fetch)
    // ------------------------------------------------------------------
    assign if_packet[1].inst         = (if_valid && two_valid_insts) ? Imem_data.word_level[1] : `NOP;
    assign if_packet[1].PC           = tb_PC + 4;
    assign if_packet[1].NPC          = tb_PC + 8;
    assign if_packet[1].valid        = if_valid && two_valid_insts && !stop_fetch;
    assign if_packet[1].predict_taken = 1'b0;
    assign if_packet[1].predict_addr  = tb_PC + 8;
